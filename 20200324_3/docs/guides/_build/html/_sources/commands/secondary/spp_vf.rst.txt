...
``forwarder`` role is for simply forwarding from source port to destination 
port.
On the other hands, ``merger`` role is for receiving packets from multiple 
ports as N:1 communication, or ``classifier`` role is for sending packet to
